...
void JavaServerClientService::runLongRunningTask() {
    ...
    Request request = mServerRequests.runLongRunningTaskRequest(a, b, list);

    mCommunicationManager.sendRequest(request);
...
//    Request request("TEST", payload);
//    qDebug() << __PRETTY_FUNCTION__ << QString(request.toByteArray());
